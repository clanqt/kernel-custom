...
unsigned int NoOfBytes, printCount;
unsigned int Temp=0;
unsigned int PrintTemp=0;
unsigned char Dummy=0;
unsigned char PrintDummy=0; //!yaswant
...
*/		
        ...
        case 41: // I - Image printing
            ...
            //            printk("buff at 1 ===>>>%c%c\n",g[6],g[7]);
            //            g[6]=(**(buff));
            //            ++(*buff);
            //            g[7]=(**(buff));
            //            ++(*buff);

            //            printk("G array after for %c%c", g[6],g[7]);
            low_bat=0;
            ...
            //            memset(PrintDummy,0x00.sizeof(PrintDummy))

            //                    printCount|=((g[6]&0x0F)<<4);
            //            printCount|=(g[7]&0x0F);

            //            PrintDummy=printCount>>4&0x0F;
            //            PrintTemp=PrintTemp*0x0A+PrintDummy;
            //            //            sprintf(filepath,"/usr/share/status/PRINTER_image%d",PrintTemp);

            //            PrintDummy=printCount&0x0F;
            //            PrintTemp=PrintTemp*0x0A+PrintDummy;
            //            //            sprintf(filepath,"/usr/share/status/PRINTER_image%d",PrintTemp);

            //            //            printCount=PrintTemp;

            //            printk("buff at 1 ===>>>%d\n",PrintTemp);

            //            if(printCount < 10)
            //            {
            //                sprintf(filepath,"/usr/share/status/PRINTER_image0%d",PrintTemp);
            //            }else{
            //                sprintf(filepath,"/usr/share/status/PRINTER_image%d",PrintTemp);
            //            }
            //            printCount = 0;
            ...
            //            struct PRINTER_image pi;

            //            printk("buff at 1 ===>>>%s\n",pi.user_buff[1]);

            //            struct printer_dev pd;

            //            printk("buff at 1 ===>>>%s\n",pd.user_buff[1]);

            //            if(printCount < 10)
            //            if((char*)g[6]=='0'){
            //                sprintf(filepath,"/usr/share/status/PRINTER_image0%c",g[7]);
            //            }else{
            //                sprintf(filepath,"/usr/share/status/PRINTER_image%c%c",g[6],g[7]);
            //            }
