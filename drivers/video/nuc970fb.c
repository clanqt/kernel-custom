...
//#define NUC970_FB_TIMEOUT	(msecs_to_jiffies(10))
...
//	timeout = wait_for_completion_interruptible_timeout(&info->completion, NUC970_FB_TIMEOUT);
...
static int nuc970fb_resume(struct platform_device *dev)
{
	...
	//writel(readl(info->io + REG_LCM_DCCS) | LCM_DCCS_VA_EN, info->io + REG_LCM_DCCS);	
	ili9341_init();
