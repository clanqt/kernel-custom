...
{
    unsigned char i;
    gpio_direction_output(38,0); 		//cs=0;
    ...
    for(i=0;i<8;i++)
    {
        if (y&0x80)
        {
            gpio_direction_output(39,0);	//sd0=0 for command
            gpio_direction_output(40,1);	//sdi=1; MOSI-D0
        }
        else
        {
            gpio_direction_output(39,0);	//scl=0;
            gpio_direction_output(40,0);	//sdi=0; MOSI=D0
        }
        gpio_direction_output(39,1);		//scl=0
        y=y<<1;
    }
    gpio_direction_output(38,1);		//cs=1;
}
/*
void read_data(void) // (uchar w, uchar v)
{
    unsigned char i;
    gpio_direction_output(38,0); 		//cs=0;
    gpio_direction_output(40,1);		//sd0=1 for data
    gpio_direction_output(39,0);		//scl=0
    udelay(2);
    gpio_direction_output(39,1);		//scl=1

    for(i=0;i<8;i++)
    {
    if(gpio_direction_input(41)==1)
        {
                printk("%d",1);
        }
        else
...
void write_data(unsigned char w) // (uchar w, uchar v)
{
    ...
    for(i=0;i<8;i++)
    {
        ...
        {
            gpio_direction_output(39,0);	        //scl=1;
            gpio_direction_output(40,0);        	// sdi=0;
        }
...
static void ili9341_init (void)
{
    gpio_request(40,"ili9341");   /* D0 */
    gpio_export(40,true);
    gpio_direction_output(40,0);

    gpio_request(41,"ili9341");   /* D1 */
    gpio_export(41,true);
    gpio_direction_input(41);

    gpio_request(38,"ili9341");   /* CS0 */
    gpio_export(38,true);
    gpio_direction_output(38,1);

    gpio_request(39," ili9341");   /* CLK */
    gpio_export(39,true);
    gpio_direction_output(39,0);

    gpio_request(37," ili9341");   /*  RESET */
    gpio_export(37,true);
    gpio_direction_output(37,0);

    gpio_request(36,"ili9341");   /* DATA/COMMAND */
    gpio_export(36,true);
    ...
    mdelay(50);
    printk("Initializing Buy Display Completed...!\n");
}
