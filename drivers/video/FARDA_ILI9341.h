...
{
    unsigned char i;
    gpio_direction_output(38,0); 		//Chip Select 0
    gpio_direction_output(36,0);          //DataCommand 0
    for(i=0;i<8;i++)
    {
        gpio_direction_output(39,0);           //Clock Pin 0
        if (y&0x80)
        {
            gpio_direction_output(40,1);	//SDA Data Output Pin
        }
        else
        {
            gpio_direction_output(40,0);	//sdi=0; MOSI=D0
        }
        gpio_direction_output(39,1);	//Clock Pin 1
        y=y<<1;
    }
    gpio_direction_output(38,1);		//Chip Select 1
}
...
void SPI_WriteData(unsigned char y) // (uchar w, uchar v)
{
    unsigned char i;
    gpio_direction_output(38,0); 		//Chip Select 0
    gpio_direction_output(36,1);          //DataCommand 0
    for(i=0;i<8;i++)
    {
        gpio_direction_output(39,0);           //Clock Pin 0
        if (y&0x80)
...
static void ili9341_init(void)
{
    gpio_request(40,"ili9488");   /* D0 */
    gpio_direction_output(40,0);

    gpio_request(41,"ili9488");  /* D1 */
    gpio_direction_input(41);

    gpio_request(38,"ili9488");  /* CS0 */
    gpio_direction_output(38,1);

    gpio_request(39,"ili9844");   /* CLK */
    gpio_direction_output(39,0);

    gpio_request(37,"ili9844");    /*  RESET */
    gpio_direction_output(37,0);

    gpio_request(36,"ili9488");    /* DATA/COMMAND */
