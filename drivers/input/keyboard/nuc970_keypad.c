...
/******************************Decleration*****************************************/
static int rotate_handler=0;
static int backspace=0,rot_a=0,rot_d=0,rot_g=0,rot_j=0,rot_m=0,rot_p=0,rot_t=0,rot_w=0;
...
char    *g_s_Hello_World_string;
ssize_t g_s_Hello_World_size;
static ssize_t time_read(
        struct file *file_ptr
        , char __user *user_buffer
        , size_t count
        , loff_t *position)
{
    switch (KEYPADMODE_state) {
    case 0:
        g_s_Hello_World_string="num\n\0";
        g_s_Hello_World_size=sizeof(g_s_Hello_World_string);
        break;
    case 1:
        g_s_Hello_World_string="sml\n\0";
        g_s_Hello_World_size=sizeof(g_s_Hello_World_string);
        break;
    case 2:
        g_s_Hello_World_string="big\n\0";
        g_s_Hello_World_size=sizeof(g_s_Hello_World_string);
        break;
    }
...
static void proc_win_exit(void)
{
...
static int nuc970_keymap[32] = {
    ...
    KEY_LEFT,           KEY_RIGHTALT,   KEY_F3,        KEY_4,
    // (2,4) 20         (2,5)  21       (2,6)  22      (2,7)  23
    KEY_5,              KEY_6,          KEY_LEFTALT,   KEY_F4,
...
static int nuc970_keymap1[32] = {
    // (0,0) 0          (0,1) 1         (0,2) 2         (0,3) 3
    KEY_LEFTSHIFT,	KEY_F8, 	KEY_F5,         KEY_F9,
    // (0,4) 4          (0,5)  5        (0,6)  6        (0,7)  7
    KEY_UP,	        KEY_F7,         KEY_TAB,	KEY_F1,
    ...
    for (i = 0; i < KEY_COUNT; i++) {
        ...
        if ((new_key ^ old_key) & (1 << i)) {// key state change
            if (new_key & (1 << i)) {
                ...
                if(KEYPADMODE_state==2) {
                    input_event(nuc970_keypad_input_dev, EV_MSC, MSC_SCAN, 0);
                    input_report_key(nuc970_keypad_input_dev, nuc970_keymap1[0], 1);//LeftShift
                    input_sync(nuc970_keypad_input_dev);
                }
                if(backspace==1) {
                    input_event(nuc970_keypad_input_dev, EV_MSC, MSC_SCAN, 28);
                    input_report_key(nuc970_keypad_input_dev, nuc970_keymap[28], 1);//Backspace
                    input_report_key(nuc970_keypad_input_dev, nuc970_keymap[28], 0);//Backspace
                    input_sync(nuc970_keypad_input_dev);
                }
                input_event(nuc970_keypad_input_dev, EV_MSC, MSC_SCAN, i);
                ...
                if(KEYPADMODE_state==2) {
                    input_event(nuc970_keypad_input_dev, EV_MSC, MSC_SCAN, 0);
                    input_report_key(nuc970_keypad_input_dev, nuc970_keymap1[0], 0);//LeftShift
                    input_sync(nuc970_keypad_input_dev);
                }
            } else {
                KEY_RELEASE_DEF(i);//Key release Definitions or down here
...
static void KEY_PRESS_DEF(int i) {
    ...
    case 16:
        backspace=0;
        reset_key();
        nuc970_keymap[i]=KEY_LEFT;
        break;
    case 17:
        backspace=0;
        reset_key();
        nuc970_keymap[i]=KEY_RIGHTALT;
        break;
    case 18:
        backspace=0;
        reset_key();
