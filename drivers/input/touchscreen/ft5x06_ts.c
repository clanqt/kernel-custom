...
//static int screenres[2] = {1280, 800}; gokul
static int screenres[2] = {320, 480};
...
static void translate(int *px, int *py)
{
	...
	//	*px = screenres[0]-x ; //Added by Rajesh for touch invert
        //       *py = screenres[1]-y ; //Added by Rajesh for touch invert
	}
}
...
static int ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	sema_init(&ts->sem, 1);
//Chages done to intailizing Interrupt and panic issue. 
	ts->client = client;
	gpio_request(225, "CTP_IRQ");
	gpio_direction_input(225);
	client->irq = gpio_to_irq(225);
	ts->irq = client->irq;
//Chages done to intailize Interrupt and panic issue.
