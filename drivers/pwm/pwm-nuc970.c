...
// Defined to read write PWM convert to gpio by gokul
u32 u32PB_MFP = 0x0;
...
static int nuc970_pwm_suspend(struct platform_device *pdev, pm_message_t state)
{
	...
	if (pdev->id == 0) {
		...
		//Added to convert PWM to GPIO
		//printk("Before Set on Standby %08x\n", __raw_readl(REG_MFP_GPB_L));
	        u32PB_MFP = readl(REG_MFP_GPB_L);
	        writel(readl(REG_MFP_GPB_L) & ~( 0xf << 8 ), REG_MFP_GPB_L); // set PB2 is GPIO mode
	        writel(readl(REG_GPIOB_DIR) | (0x1 << 2), REG_GPIOB_DIR);  // Set PB2 is output mode
	        writel(readl(REG_GPIOB_DATAOUT) & ~(0x1 << 2), REG_GPIOB_DATAOUT);  // Set PB2 output Low
	        //printk("Before Set off Standby %08x\n", __raw_readl(REG_MFP_GPB_L));
		//Added to convert PWM to GPIO
	}
...
static int nuc970_pwm_resume(struct platform_device *pdev)
{
	...
	if (pdev->id == 0) {
		...
		//Added to convert GPIO to PWM
		//printk("After Set on Standby %08x\n", __raw_readl(REG_MFP_GPB_L));
//		writel(0xbb000d00, REG_MFP_GPB_L); // set PB2 is GPIO PWM mode
		writel(0x00000d00, REG_MFP_GPB_L); // set PB2 is GPIO PWM mode
		//printk("After Set off Standby %08x\n", __raw_readl(REG_MFP_GPB_L));
		//Added to convert GPIO to PWM
	}
	else if (pdev->id == 1) {
