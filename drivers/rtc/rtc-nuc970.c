...
static int nuc970_rtc_set_alarm(struct device *dev, struct rtc_wkalrm *alrm)
{
    ...
    printk("Before Reg Write: 0x%.8X\n", __raw_readl(rtc->rtc_reg + REG_RTC_PWRCTL));
    err = check_rtc_access_enable(rtc);
    if (IS_ERR(err))
        return PTR_ERR(err);
    rtc_reg_write(rtc, REG_RTC_PWRCTL, (__raw_readl(rtc->rtc_reg + REG_RTC_PWRCTL)) & 0xFFFFFFFE);
    printk("After Reg Write: 0x%.8X\n", __raw_readl(rtc->rtc_reg + REG_RTC_PWRCTL));
